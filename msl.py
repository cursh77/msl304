...
# ---------------------------------------
# TASKS COMPLETED BY STAFF (Bottom Section)
# ---------------------------------------
if st.session_state.schedule:
    st.divider()
    st.subheader("📊 Tasks Completed by Staff")
    schedule_df = pd.DataFrame(st.session_state.schedule)
    staff_summary = schedule_df.groupby('Staff').agg({
        'Job ID': 'count',
        'Priority': 'sum'
    }).reset_index()
    staff_summary.columns = ['Staff Member', 'Tasks Completed', 'Total Priority']
    staff_summary = staff_summary.sort_values('Tasks Completed', ascending=False)
